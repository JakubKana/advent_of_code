...
const inputLoaderPart1 = new InputLoader().setPath(
  path.join(getDirname(), getPart1InputPath())
);
const inputLoaderPart2 = new InputLoader().setPath(
  path.join(getDirname(), getPart2InputPath())
);
...
function isPrintedNumberAfterAllRules(
  printedNumber: number,
  printedNumbers: number[],
  rulesForNumber: number[]
): boolean {
  let result = false;
...
function isPrintedNumberBeforeAllRules(
  printedNumber: number,
  printedNumbers: number[],
  rulesForNumber: number[]
): boolean {
...
function getMinPositionBasedOnAfterRulesIndex(
  printedLine: number[],
  rulesForNumber: number[]
): number {
  let index = -1;
...
function getMaxPositionBasedOnBeforeRulesIndex(
  printedLine: number[],
  rulesForNumber: number[]
): number {
  let index = -1;
...
function moveElementInArrayByIndex(
  arr: number[],
  fromIndex: number,
  toIndex: number
) {
...
function runPart1() {
  ...
  printLines.forEach((line) => {
    ...
    for (const printedNumber of printedNumbersLine) {
      ...
      const canNotPrintBeforeNumbers =
        canNotPrintBeforeRules.get(printedNumber);
      ...
      if (canNotPrintAfterNumbers || canNotPrintBeforeNumbers) {
        ...
        const isPrintedNumberBeforeAll = isPrintedNumberBeforeAllRules(
          printedNumber,
          printedNumbersLine,
          canNotPrintAfterNumbers || []
        );
        const isPrintedNumberAfterAll = isPrintedNumberAfterAllRules(
          printedNumber,
          printedNumbersLine,
          canNotPrintBeforeNumbers || []
        );
    ...
    if (!correctlyPrinted) {
      ...
      for (const printedNumber of printedNumbersLine) {
        const canNotPrintAfterNumbers =
          canNotPrintAfterRules.get(printedNumber) || [];
        const canNotPrintBeforeNumbers =
          canNotPrintBeforeRules.get(printedNumber) || [];
        reconstructedLine.push(printedNumber);

        let hasError =
          !isPrintedNumberBeforeAllRules(
            printedNumber,
            reconstructedLine,
            canNotPrintAfterNumbers || []
          ) ||
          !isPrintedNumberAfterAllRules(
            printedNumber,
            reconstructedLine,
            canNotPrintBeforeNumbers || []
          );
        do {
          if (hasError) {
            const minIndex = getMinPositionBasedOnAfterRulesIndex(
              reconstructedLine,
              canNotPrintAfterNumbers
            );
            const maxIndex = getMaxPositionBasedOnBeforeRulesIndex(
              reconstructedLine,
              canNotPrintBeforeNumbers
            );
            const printedNumberIndex = reconstructedLine.indexOf(printedNumber);
    ...
    if (correctlyPrinted) {
      ...
      correctlyPrintedSum +=
        printedNumbersLine[Math.floor(printedNumbersLine.length / 2)];
    } else {
      results.push(reconstructedLine);
      incorrectlyPrintedSum +=
        reconstructedLine[Math.floor(reconstructedLine.length / 2)];
    }
  });
